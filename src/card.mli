(** Representation of cards in the poker game

    This module represents the remaining cards that have not been dealt
    yet, which includes initializing a new deck of cards, drawing cards
    randomly from the deck of remaining cards, and showing cards to
    players by printing them out in a string. It handles comparing hands
    of players at the end of the game as well as estimating the strength
    of hands which is an important piece of information to bot *)

(** The abstract type of card. The suits are [S]pades, [H]earts, [C]lubs
    [D]iamonds. Examples:
...
(** [rank_of_hand hand] returns the rank of [hand] according to texas
    ...
    docs/card_ranking_rules.txt. Requires: 5 <= [List.length hand] and
    [List.length hand] <= 7. *)
...
(** [starting_hand_estimated_strength hand] returns the estimated
    ...
    the strongest and 0 the weakest. Requires: [List.length hand] = 2. *)
...
(** [index_of_highest_hand card_list_list] returns the index of the
    ...
    rules as specified in docs/card_ranking_rules.txt. Requires: for any
    element e in [card_list_list], [List.length e] = 7. Raises: [Tie] of
    \[i1 ... in \] when hands at index i1 ... in in [card_list_list] are
    tied. *)
